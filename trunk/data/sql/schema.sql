CREATE TABLE areas_formacion (id BIGSERIAL, nombre_area VARCHAR(50), PRIMARY KEY(id));
CREATE TABLE areas_formacion_facilitador (id BIGSERIAL, id_identificacion INT, id_area_formacion INT, estatus VARCHAR(20), PRIMARY KEY(id));
CREATE TABLE bitacora_formacion_facilitador (id BIGSERIAL, id_area_formacion_facilitador INT, fecha DATE, PRIMARY KEY(id));
CREATE TABLE correos (id BIGSERIAL, id_identificacion INT, correo VARCHAR(50) NOT NULL, PRIMARY KEY(id));
CREATE TABLE disponibilidad_dias (id BIGSERIAL, id_identificacion INT, dia VARCHAR(20), PRIMARY KEY(id));
CREATE TABLE disponibilidad_traslado_estado (id BIGSERIAL, id_identificacion INT, id_estado INT, id_municipio INT, requiere_traslado BOOLEAN, PRIMARY KEY(id));
CREATE TABLE disponibilidad_turnos (id BIGSERIAL, id_disponibilidad_dia INT, turno VARCHAR(20), PRIMARY KEY(id));
CREATE TABLE ente (id BIGSERIAL, nombre_ente VARCHAR(50), id_estado INT, id_municipio INT, id_parroquia INT, PRIMARY KEY(id));
CREATE TABLE estado (id BIGSERIAL, nombre_estado VARCHAR(20) NOT NULL, PRIMARY KEY(id));
CREATE TABLE estudios (id BIGSERIAL, nombre_estudio VARCHAR(50), PRIMARY KEY(id));
CREATE TABLE identificacion (id BIGSERIAL, nombre VARCHAR(50) NOT NULL, apellido VARCHAR(50), cedula_pasaporte INT NOT NULL, nacionalidad VARCHAR(20), direccion VARCHAR(100), sector VARCHAR(100), situacion_laboral VARCHAR(50), formacion_politica BOOLEAN, id_estado INT, id_municipio INT, id_parroquia INT, PRIMARY KEY(id));
CREATE TABLE municipio (id BIGSERIAL, nombre_municipio VARCHAR(50), id_estado INT, PRIMARY KEY(id));
CREATE TABLE nivel_formacion (id BIGSERIAL, id_identificacion INT, id_estudios INT, PRIMARY KEY(id));
CREATE TABLE ocupacion (id BIGSERIAL, id_identificacion INT, nombre_ocupacion VARCHAR(50), PRIMARY KEY(id));
CREATE TABLE parroquia (id BIGSERIAL, nombre_parroquia VARCHAR(50), id_municipio INT, PRIMARY KEY(id));
CREATE TABLE profesion (id BIGSERIAL, id_identificacion INT, nombre_profesion VARCHAR(50), PRIMARY KEY(id));
CREATE TABLE secciones (id BIGSERIAL, id_identificacion INT, id_area_formacion_facilitador INT, nombre_seccion VARCHAR(50), id_ente INT, PRIMARY KEY(id));
CREATE TABLE telefonos (id BIGSERIAL, id_identificacion INT, numero INT, PRIMARY KEY(id));
